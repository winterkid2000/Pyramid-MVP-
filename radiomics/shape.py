...
def run_combined_descriptor(nifti_ct_path, nifti_mask_path, stl_path, output_csv_path, log_callback=None) -> bool:
    try:
        if log_callback:
            log_callback(f"히스토그램 계산은 즐거워요")
        ...
        if ct.shape != mask.shape:
            if log_callback:
                log_callback("남의 마스크 훔치지 마요")
            return False

        if log_callback:
            log_callback(f"마스크 고유값을 확인해 봅시다: {np.unique(mask)}")
            log_callback(f"참고 사항을 말해보자! 최소 HU={np.min(ct)}, 최대 HU={np.max(ct)}")
        ...
        for i, (low, high) in enumerate(bins, start=1):
            ...
            percentage = (count / total_voxels) if total_voxels > 0 else 0
            hu_hist_features[f"HU_Histogram_{i}"] = round(percentage, 4)

        if log_callback:
            log_callback(f"네모빔: {os.path.basename(stl_path)}")
        ...
        if log_callback:
            log_callback(f"신상 조사 끝!: {output_csv_path}")

        return True

    except Exception as e:
        if log_callback:
            log_callback(f"신상 털이범이 되었습니다...: {str(e)}")
        return False
